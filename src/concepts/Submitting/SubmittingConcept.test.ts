...
describe("Submitting", () => {
  ...
  test("submit increments attempt number for same (assignment, submitter)", async () => {
    ...
    const r1 = await Submitting._getSubmission({
      submission: first.submission,
    });
    expect(r1[0]?.number).toBe(1);
    const r2 = await Submitting._getSubmission({
  ...
  test("restore fails on non-existent submission", async () => {
    expect(
      await Submitting.restore({ submission: "nonexistent" as ID }),
    ).toHaveProperty("error");
  });

  test("withdraw fails on non-existent submission", async () => {
    expect(
      await Submitting.withdraw({ submission: "nonexistent" as ID }),
    ).toHaveProperty("error");
  });
  ...
  test("_getAttempts returns all attempts sorted", async () => {
    ...
    const attempts = await Submitting._getAttempts({
      assignment: a,
      submitter: s,
    });
  ...
  test("_hasSubmission returns true/false correctly", async () => {
    ...
    const s = submitter("alice");
    expect(
      await Submitting._hasSubmission({ assignment: a, submitter: s }),
  ...
  test("_getSubmissionsForAssignment filters by assignment", async () => {
    ...
    const forA1 = await Submitting._getSubmissionsForAssignment({
      assignment: a1,
    });
    expect(forA1).toHaveLength(2);
    expect(forA1.map((r) => r.submitter)).toContain(s1);
    expect(forA1.map((r) => r.submitter)).toContain(s2);
    const forA2 = await Submitting._getSubmissionsForAssignment({
      assignment: a2,
    });
  ...
  test("_getSubmission returns empty array for non-existent submission", async () => {
    const result = await Submitting._getSubmission({
      submission: "nonexistent" as ID,
    });
  ...
  test("attempt numbers are independent across different assignments", async () => {
    ...
    const result = ok(
      await Submitting.submit({
        assignment: a2,
        submitter: s,
        artifacts: [artifact("b1")],
      }),
    );
    const details = await Submitting._getSubmission({
  ...
  test("namespaces isolate duplicate concept instances", async () => {
    ...
    expect(
      await Drafts._hasSubmission({ assignment: a, submitter: s }),
    ).toEqual([{ submitted: true }]);
    expect(
      await Finals._hasSubmission({ assignment: a, submitter: s }),
    ).toEqual([{ submitted: true }]);
    expect(
      await Submitting._hasSubmission({ assignment: a, submitter: s }),
    ).toEqual([{ submitted: false }]);
