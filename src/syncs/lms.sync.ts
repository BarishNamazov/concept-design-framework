...
async function resolveSession(frames: Frames, session: symbol, fu: symbol): Promise<Frames> {
  return await frames.query(Sessioning._getUser, { session }, { user: fu });
}

async function requireCapability(
  frames: Frames, fu: symbol, cap: string, fa: symbol,
): Promise<Frames> {
  frames = await frames.query(
    Roling._hasCapability, { user: fu, context: FORUM_CONTEXT, capability: cap }, { allowed: fa },
  );
  return frames.filter(($) => $[fa] === true);
}

async function requireActiveStudent(
  frames: Frames, session: symbol, fu: symbol, fa: symbol,
): Promise<Frames> {
  frames = await resolveSession(frames, session, fu);
  frames = await frames.query(Rostering._isActiveStudent, { user: fu }, { active: fa });
  return frames.filter(($) => $[fa] === true);
}

async function requireStaff(
  frames: Frames, session: symbol, fu: symbol, cap: string, fa: symbol,
): Promise<Frames> {
  frames = await resolveSession(frames, session, fu);
...
const configureClass = defineEndpoint("/roster/configure-class", ({ Sync, Actions, Request, Respond, Fail }) => ({
  RosterConfigureClass: Sync(({ session, code, title, term, timezone, fu, fa }) => ({
    when: Actions(Request({ session, code, title, term, timezone })),
    where: async (f) => await requireStaff(f, session, fu, ROSTER_MANAGE, fa),
    then: Actions([Rostering.configureClass, { code, title, term, timezone }]),
  })),
...
const rosterMe = defineEndpoint("/roster/me", ({ Sync, Actions, Request, Respond }) => ({
...
const rosterSectionsList = defineEndpoint("/roster/sections/list", ({ Sync, Actions, Request, Respond }) => ({
  RosterSectionsList: Sync(({ section, name, location, meetingPattern, status, sections }) => ({
    when: Actions(Request()),
    where: async (f) => {
      const [base] = f;
      f = await f.query(Rostering._getSections, {}, { section, name, location, meetingPattern, status });
      return f.aggregate(base, [section, name, location, meetingPattern, status], sections);
    },
    then: Actions(Respond({ sections })),
  })),
}));

const rosterSectionsCreate = defineEndpoint("/roster/sections/create", ({ Sync, Actions, Request, Respond, Fail }) => ({
  RosterSectionsCreate: Sync(({ session, name, location, meetingPattern, fu, fa }) => ({
    when: Actions(Request({ session, name, location, meetingPattern })),
    where: async (f) => await requireStaff(f, session, fu, ROSTER_MANAGE, fa),
    then: Actions([Rostering.createSection, { name, location, meetingPattern }]),
  })),
...
const rosterSectionsUpdate = defineEndpoint("/roster/sections/update", ({ Sync, Actions, Request, Respond, Fail }) => ({
  RosterSectionsUpdate: Sync(({ session, section, name, location, meetingPattern, fu, fa }) => ({
    when: Actions(Request({ session, section, name, location, meetingPattern })),
    where: async (f) => await requireStaff(f, session, fu, ROSTER_MANAGE, fa),
    then: Actions([Rostering.updateSection, { section, name, location, meetingPattern }]),
  })),
...
const rosterImportPreview = defineEndpoint("/roster/import-preview", ({ Sync, Actions, Request, Respond }) => ({
  RosterImportPreview: Sync(({ csv, rows }) => ({
    ...
    where: async (f) =>
      f.map(($) => {
        ...
        if (lines.length < 2) return { ...$, [rows]: [] as Record<string, string>[] };
...
const rosterImport = defineEndpoint("/roster/import", ({ Sync, Actions, Request, Respond, Fail }) => ({
  RosterImport: Sync(({ session, rows, fu, fa, ek, em, rn, kd, sc }) => ({
    ...
    then: Actions([Rostering.importSeat, { externalKey: ek, email: em, rosterName: rn, kind: kd, section: sc }]),
  })),
...
const rosterClaimSeat = defineEndpoint("/roster/claim-seat", ({ Sync, Actions, Request, Respond, Fail }) => ({
  RosterClaimSeat: Sync(({ session, externalKey, fu, seat }) => ({
    when: Actions(Request({ session, externalKey })),
    where: async (f) => {
      f = await resolveSession(f, session, fu);
      return await f.query(Rostering._getSeatByExternalKey, { externalKey }, { seat });
    },
...
const rosterLinkUser = defineEndpoint("/roster/link-user", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const rosterList = defineEndpoint("/roster/list", ({ Sync, Actions, Request, Respond }) => ({
  RosterList: Sync(({ session, fu, fa, user, seat, kind, section, rosterName, email, members }) => ({
    when: Actions(Request({ session })),
    where: async (f) => {
      const [base] = f;
      f = await requireStaff(f, session, fu, ROSTER_MANAGE, fa);
      f = await f.query(Rostering._getActiveMembers, {}, { user, seat, kind, section, rosterName, email });
      return f.aggregate(base, [user, seat, kind, section, rosterName, email], members);
    },
    then: Actions(Respond({ members })),
  })),
}));

const rosterDrop = defineEndpoint("/roster/drop", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const rosterReinstate = defineEndpoint("/roster/reinstate", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const rosterMoveSection = defineEndpoint("/roster/move-section", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
export const rosterApi = {
  configureClass,
  me: rosterMe,
  sections: { list: rosterSectionsList, create: rosterSectionsCreate, update: rosterSectionsUpdate },
...
const assignmentsCreateDraft = defineEndpoint("/assignments/create-draft", ({ Sync, Actions, Request, Respond, Fail }) => ({
  AssignmentsCreateDraft: Sync(({
    session, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets, fu, fa,
  }) => ({
    when: Actions(Request({
      session, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets,
    })),
    where: async (f) => await requireStaff(f, session, fu, ASSIGNMENTS_MANAGE, fa),
    then: Actions([Assigning.createDraft, {
      author: fu, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets,
    }]),
  })),
...
const assignmentsRevise = defineEndpoint("/assignments/revise", ({ Sync, Actions, Request, Respond, Fail }) => ({
  AssignmentsRevise: Sync(({
    session, assignment, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets, fu, fa,
  }) => ({
    when: Actions(Request({
      session, assignment, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets,
    })),
    where: async (f) => await requireStaff(f, session, fu, ASSIGNMENTS_MANAGE, fa),
    then: Actions([Assigning.revise, {
      assignment, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets,
    }]),
  })),
...
const assignmentsPublish = defineEndpoint("/assignments/publish", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const assignmentsArchive = defineEndpoint("/assignments/archive", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const assignmentsForMe = defineEndpoint("/assignments/for-me", ({ Sync, Actions, Request, Respond }) => ({
  AssignmentsForMe: Sync(({ session, fu, fa, assignment, release, dueOverride, status, assigned }) => ({
    when: Actions(Request({ session })),
    where: async (f) => {
      const [base] = f;
      f = await requireActiveStudent(f, session, fu, fa);
      f = await f.query(Assigning._getAssigned, { assignee: fu }, { assignment, release, dueOverride, status });
      return f.aggregate(base, [assignment, release, dueOverride, status], assigned);
    },
    then: Actions(Respond({ assignments: assigned })),
  })),
}));

const assignmentsGet = defineEndpoint("/assignments/get", ({ Sync, Actions, Request, Respond }) => ({
  AssignmentsGet: Sync(({ assignment, asmtAssignment, author, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets, status, createdAt, updatedAt, detail }) => ({
    when: Actions(Request({ assignment })),
    where: async (f) => {
      const [base] = f;
      f = await f.query(Assigning._getAssignment, { assignment }, { assignment: asmtAssignment, author, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets, status, createdAt, updatedAt });
      return f.aggregate(base, [asmtAssignment, author, title, instructions, kind, availableAt, dueAt, closeAt, acceptsSubmissions, audience, targets, status, createdAt, updatedAt], detail);
    },
    then: Actions(Respond({ assignment: detail })),
  })),
}));

const assignmentsStaffSummary = defineEndpoint("/assignments/staff-summary", ({ Sync, Actions, Request, Respond }) => ({
  AssignmentsStaffSummary: Sync(({ session, assignment, fu, fa, detail }) => ({
    when: Actions(Request({ session, assignment })),
    where: async (f) => {
      f = await requireStaff(f, session, fu, ASSIGNMENTS_MANAGE, fa);
      return await f.query(Assigning._getAssignment, { assignment }, { detail });
    },
    then: Actions(Respond({ summary: detail })),
  })),
}));

const assignmentsSetDueOverride = defineEndpoint("/assignments/set-due-override", ({ Sync, Actions, Request, Respond, Fail }) => ({
  AssignmentsSetDueOverride: Sync(({ session, assignment, assignee, dueAt, fu, fa }) => ({
    when: Actions(Request({ session, assignment, assignee, dueAt })),
    where: async (f) => await requireStaff(f, session, fu, ASSIGNMENTS_MANAGE, fa),
    then: Actions([Assigning.setDueOverride, { assignment, assignee, dueAt }]),
  })),
...
const assignmentsClearDueOverride = defineEndpoint("/assignments/clear-due-override", ({ Sync, Actions, Request, Respond, Fail }) => ({
  AssignmentsClearDueOverride: Sync(({ session, assignment, assignee, fu, fa }) => ({
...
const assignmentsSubmit = defineEndpoint("/assignments/submit", ({ Sync, Actions, Request, Respond, Fail }) => ({
  AssignmentsSubmitCreatePost: Sync(({ session, assignment, content, fu, fa, post }) => ({
  ...
  AssignmentsSubmitDo: Sync(({ assignment, fu, post }) => ({
    ...
    then: Actions([Submitting.submit, { assignment, submitter: fu, artifact: post }]),
  })),
...
const submissionsLatest = defineEndpoint("/submissions/latest", ({ Sync, Actions, Request, Respond }) => ({
  SubmissionsLatest: Sync(({ assignment, submitter, result }) => ({
    when: Actions(Request({ assignment, submitter })),
    where: async (f) => await f.query(Submitting._getLatest, { assignment, submitter }, { result }),
    then: Actions(Respond({ submission: result })),
  })),
}));

const submissionsAttempts = defineEndpoint("/submissions/attempts", ({ Sync, Actions, Request, Respond }) => ({
  SubmissionsAttempts: Sync(({ assignment, submitter, result }) => ({
    when: Actions(Request({ assignment, submitter })),
    where: async (f) => await f.query(Submitting._getAttempts, { assignment, submitter }, { result }),
    then: Actions(Respond({ attempts: result })),
  })),
}));

const submissionsForAssignment = defineEndpoint("/submissions/for-assignment", ({ Sync, Actions, Request, Respond }) => ({
  SubmissionsForAssignment: Sync(({ session, assignment, fu, fa, result }) => ({
    when: Actions(Request({ session, assignment })),
    where: async (f) => {
      f = await requireStaff(f, session, fu, SUBMISSIONS_VIEW_ALL, fa);
      return await f.query(Submitting._getSubmissionsForAssignment, { assignment }, { result });
    },
    then: Actions(Respond({ submissions: result })),
  })),
}));

const submissionsForStudent = defineEndpoint("/submissions/for-student", ({ Sync, Actions, Request, Respond }) => ({
  SubmissionsForStudent: Sync(({ submitter, result }) => ({
    when: Actions(Request({ submitter })),
    where: async (f) => await f.query(Submitting._getSubmissionsForSubmitter, { submitter }, { result }),
    then: Actions(Respond({ submissions: result })),
  })),
}));
...
const lateDaysConfigurePolicy = defineEndpoint("/late-days/configure-policy", ({ Sync, Actions, Request, Respond, Fail }) => ({
  LateDaysConfigurePolicy: Sync(({ session, defaultDays, unitHours, maxDaysPerItem, fu, fa }) => ({
    when: Actions(Request({ session, defaultDays, unitHours, maxDaysPerItem })),
    where: async (f) => await requireStaff(f, session, fu, LATE_DAYS_MANAGE, fa),
    then: Actions([LateBanking.configurePolicy, { defaultDays, unitHours, maxDaysPerItem }]),
  })),
...
const lateDaysBalance = defineEndpoint("/late-days/balance", ({ Sync, Actions, Request, Respond }) => ({
  LateDaysBalance: Sync(({ learner, result }) => ({
    when: Actions(Request({ learner })),
    where: async (f) => await f.query(LateBanking._getBalance, { learner }, { result }),
    then: Actions(Respond({ balance: result })),
  })),
}));

const lateDaysApply = defineEndpoint("/late-days/apply", ({ Sync, Actions, Request, Respond, Fail }) => ({
  LateDaysApply: Sync(({ session, assignment, days, fu, fa }) => ({
    when: Actions(Request({ session, assignment, days })),
    where: async (f) => await requireActiveStudent(f, session, fu, fa),
    then: Actions([LateBanking.apply, { learner: fu, item: assignment, days }]),
  })),
...
const lateDaysChange = defineEndpoint("/late-days/change", ({ Sync, Actions, Request, Respond, Fail }) => ({
  LateDaysChange: Sync(({ session, assignment, days, fu }) => ({
    when: Actions(Request({ session, assignment, days })),
    where: async (f) => await resolveSession(f, session, fu),
    then: Actions([LateBanking.changeUse, { learner: fu, item: assignment, days }]),
  })),
...
const lateDaysCancel = defineEndpoint("/late-days/cancel", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const lateDaysGrant = defineEndpoint("/late-days/grant", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const lateDaysList = defineEndpoint("/late-days/list", ({ Sync, Actions, Request, Respond }) => ({
...
const lateDaysForAssignment = defineEndpoint("/late-days/for-assignment", ({ Sync, Actions, Request, Respond }) => ({
  LateDaysForAssignment: Sync(({ session, assignment, fu, fa, result }) => ({
    when: Actions(Request({ session, assignment })),
    where: async (f) => {
      f = await requireStaff(f, session, fu, LATE_DAYS_MANAGE, fa);
      return await f.query(LateBanking._getUsersForItem, { item: assignment }, { result });
    },
...
const gradesConfigureItem = defineEndpoint("/grades/configure-item", ({ Sync, Actions, Request, Respond, Fail }) => ({
  GradesConfigureItem: Sync(({ session, item, label, maxPoints, fu, fa }) => ({
...
const gradesAddCriterion = defineEndpoint("/grades/add-criterion", ({ Sync, Actions, Request, Respond, Fail }) => ({
  GradesAddCriterion: Sync(({ session, item, name, maxPoints, position, fu, fa }) => ({
    when: Actions(Request({ session, item, name, maxPoints, position })),
    where: async (f) => await requireStaff(f, session, fu, GRADES_MANAGE, fa),
    then: Actions([Grading.addCriterion, { item, name, maxPoints, position }]),
  })),
...
const gradesReviseCriterion = defineEndpoint("/grades/revise-criterion", ({ Sync, Actions, Request, Respond, Fail }) => ({
  GradesReviseCriterion: Sync(({ session, criterion, name, maxPoints, position, fu, fa }) => ({
    when: Actions(Request({ session, criterion, name, maxPoints, position })),
    where: async (f) => await requireStaff(f, session, fu, GRADES_MANAGE, fa),
    then: Actions([Grading.reviseCriterion, { criterion, name, maxPoints, position }]),
  })),
...
const gradesRemoveCriterion = defineEndpoint("/grades/remove-criterion", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const gradesRecord = defineEndpoint("/grades/record", ({ Sync, Actions, Request, Respond, Fail }) => ({
  GradesRecord: Sync(({ session, learner, item, evidence, score, feedback, fu, fa }) => ({
    when: Actions(Request({ session, learner, item, evidence, score, feedback })),
    where: async (f) => await requireStaff(f, session, fu, GRADES_MANAGE, fa),
    then: Actions([Grading.recordDraft, { learner, item, evidence, grader: fu, score, feedback }]),
  })),
...
const gradesScoreCriterion = defineEndpoint("/grades/score-criterion", ({ Sync, Actions, Request, Respond, Fail }) => ({
  GradesScoreCriterion: Sync(({ session, learner, item, criterion, points, feedback, fu, fa }) => ({
    when: Actions(Request({ session, learner, item, criterion, points, feedback })),
    where: async (f) => await requireStaff(f, session, fu, GRADES_MANAGE, fa),
    then: Actions([Grading.scoreCriterion, { learner, item, criterion, grader: fu, points, feedback }]),
  })),
...
const gradesRelease = defineEndpoint("/grades/release", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const gradesReleaseItem = defineEndpoint("/grades/release-item", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const gradesRetract = defineEndpoint("/grades/retract", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const gradesExcuse = defineEndpoint("/grades/excuse", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const gradesForMe = defineEndpoint("/grades/for-me", ({ Sync, Actions, Request, Respond }) => ({
  GradesForMe: Sync(({ session, fu, fa, result }) => ({
    when: Actions(Request({ session })),
    where: async (f) => {
      f = await requireActiveStudent(f, session, fu, fa);
      return await f.query(Grading._getGradesForLearner, { learner: fu }, { result });
    },
    then: Actions(Respond({ grades: result })),
  })),
}));

const gradesForStudent = defineEndpoint("/grades/for-student", ({ Sync, Actions, Request, Respond }) => ({
  GradesForStudent: Sync(({ session, learner, fu, fa, result }) => ({
    ...
    where: async (f) => {
      ...
      return await f.query(Grading._getGradesForLearner, { learner }, { result });
    },
    then: Actions(Respond({ grades: result })),
  })),
}));

const gradesForItem = defineEndpoint("/grades/for-item", ({ Sync, Actions, Request, Respond }) => ({
...
const gradesGradebook = defineEndpoint("/grades/gradebook", ({ Sync, Actions, Request, Respond }) => ({
...
const gradesExport = defineEndpoint("/grades/export", ({ Sync, Actions, Request, Respond }) => ({
...
const studentsDetail = defineEndpoint("/students/detail", ({ Sync, Actions, Request, Respond }) => ({
  StudentsDetail: Sync(({ session, user: learner, fu, fa, result }) => ({
    ...
    where: async (f) => {
      ...
      return await f.query(Rostering._getSeatByUser, { user: learner }, { result });
    },
    then: Actions(Respond({ detail: result })),
  })),
}));

const studentsNotesWrite = defineEndpoint("/students/notes/write", ({ Sync, Actions, Request, Respond, Fail }) => ({
  StudentsNotesWrite: Sync(({ session, learner, body, visibility, tags, followUpAt, fu, fa }) => ({
    when: Actions(Request({ session, learner, body, visibility, tags, followUpAt })),
    where: async (f) => await requireStaff(f, session, fu, STUDENT_NOTES_MANAGE, fa),
    then: Actions([StudentNoting.write, { author: fu, learner, body, visibility, tags, followUpAt }]),
  })),
...
const studentsNotesRevise = defineEndpoint("/students/notes/revise", ({ Sync, Actions, Request, Respond, Fail }) => ({
  StudentsNotesRevise: Sync(({ session, note, body, visibility, tags, followUpAt, fu, fa }) => ({
    when: Actions(Request({ session, note, body, visibility, tags, followUpAt })),
    where: async (f) => await requireStaff(f, session, fu, STUDENT_NOTES_MANAGE, fa),
    then: Actions([StudentNoting.revise, { note, body, visibility, tags, followUpAt }]),
  })),
...
const studentsNotesResolve = defineEndpoint("/students/notes/resolve", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const studentsNotesArchive = defineEndpoint("/students/notes/archive", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const studentsNotesList = defineEndpoint("/students/notes/list", ({ Sync, Actions, Request, Respond }) => ({
  StudentsNotesList: Sync(({ session, learner, fu, fa, result }) => ({
    when: Actions(Request({ session, learner })),
    where: async (f) => {
      f = await requireStaff(f, session, fu, STUDENT_NOTES_MANAGE, fa);
      return await f.query(StudentNoting._getActiveStaffNotes, { learner }, { result });
    },
    then: Actions(Respond({ notes: result })),
  })),
}));

const studentsNotesVisible = defineEndpoint("/students/notes/visible", ({ Sync, Actions, Request, Respond }) => ({
  StudentsNotesVisible: Sync(({ session, fu, fa, result }) => ({
    when: Actions(Request({ session })),
    where: async (f) => {
      f = await requireActiveStudent(f, session, fu, fa);
      return await f.query(StudentNoting._getLearnerVisibleNotes, { learner: fu }, { result });
    },
    then: Actions(Respond({ notes: result })),
  })),
}));

const studentsNotesAcknowledge = defineEndpoint("/students/notes/acknowledge", ({ Sync, Actions, Request, Respond, Fail }) => ({
...
const lmsMe = defineEndpoint("/lms/me", ({ Sync, Actions, Request, Respond }) => ({
  LmsMe: Sync(({ session, fu, fa, seat, user, externalKey, email, rosterName, kind, section, status, dashboard }) => ({
    when: Actions(Request({ session })),
    where: async (f) => {
      const [base] = f;
      f = await requireActiveStudent(f, session, fu, fa);
      f = await f.query(Rostering._getSeatByUser, { user: fu }, { seat, user, externalKey, email, rosterName, kind, section, status });
      return f.aggregate(base, [seat, user, externalKey, email, rosterName, kind, section, status], dashboard);
    },
    then: Actions(Respond({ dashboard })),
  })),
}));

const lmsStaffDashboard = defineEndpoint("/lms/staff-dashboard", ({ Sync, Actions, Request, Respond }) => ({
  LmsStaffDashboard: Sync(({ session, fu, fa, user, seat, kind, section, rosterName, email, dashboard }) => ({
    when: Actions(Request({ session })),
    where: async (f) => {
      const [base] = f;
      f = await requireStaff(f, session, fu, "roster:manage", fa);
      f = await f.query(Rostering._getActiveMembers, {}, { user, seat, kind, section, rosterName, email });
      return f.aggregate(base, [user, seat, kind, section, rosterName, email], dashboard);
    },
    then: Actions(Respond({ dashboard })),
  })),
}));
...
const calendarMe = defineEndpoint("/calendar/me", ({ Sync, Actions, Request, Respond }) => ({
  CalendarMe: Sync(({ session, start, end, fu, fa, assignment, events }) => ({
    when: Actions(Request({ session, start, end })),
    where: async (f) => {
      const [base] = f;
      f = await requireActiveStudent(f, session, fu, fa);
      f = await f.query(Assigning._getPublishedInWindow, { start, end }, { assignment });
...
const calendarStaff = defineEndpoint("/calendar/staff", ({ Sync, Actions, Request, Respond }) => ({
  CalendarStaff: Sync(({ session, start, end, section, fu, fa, result }) => ({
    ...
    where: async (f) => {
      ...
      return await f.query(Assigning._getPublishedInWindow, { start, end }, { result });
    },
