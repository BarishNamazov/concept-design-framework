...
export async function loadStaffDashboard(session: string) {
  return unwrap(
    await (api.lms as Record<string, Function>)["staff-dashboard"]({ session }),
  );
}
...
export async function loadAssignments(session: string) {
  return unwrap(
    await (api.assignments as Record<string, Function>)["for-me"]({ session }),
  );
}
...
export async function loadSubmissionsForAssignment(
  ...
  return unwrap(
    await (api.submissions as Record<string, Function>)["for-assignment"]({
      session,
      assignment,
    }),
  );
...
export async function loadSubmissionsForStudent(submitter: string) {
  return unwrap(
    await (api.submissions as Record<string, Function>)["for-student"]({
      submitter,
    }),
  );
}
...
export async function loadGradesForMe(session: string) {
  return unwrap(
    await (api.grades as Record<string, Function>)["for-me"]({ session }),
  );
}

export async function loadGradesForStudent(session: string, learner: string) {
  return unwrap(
    await (api.grades as Record<string, Function>)["for-student"]({
      session,
      learner,
    }),
  );
...
export async function loadGradesForItem(session: string, item: string) {
  return unwrap(
    await (api.grades as Record<string, Function>)["for-item"]({
      session,
      item,
    }),
  );
...
const lateDays = () =>
  api as unknown as Record<string, Record<string, Function>>;
...
export async function loadLateDaysForAssignment(
  ...
  return unwrap(
    await lateDays()["late-days"]["for-assignment"]({ session, assignment }),
  );
}

const studentsApi = () =>
  api.students as unknown as Record<string, Record<string, Function>>;
...
export async function loadStudentDetail(session: string, user: string) {
  return unwrap(
    await (api.students as unknown as Record<string, Function>).detail({
      session,
      user,
    }),
  );
